...
function Dropdown() {
  ...
  _.each(DROPDOWN.TRIGGER.ALL, function (ELEMENT) {
    if (ELEMENT.parentNode.hasAttribute('data-dropdown-animation') && ELEMENT.parentNode.getAttribute('data-dropdown-animation') === 'slide') {
      var DROPDOWN_CONTENT = ELEMENT.nextElementSibling;
...
function Tabs() {
  ...
  function Nav(event) {
    ...
    if (event.target === event.currentTarget || event.target.parentNode.classList.contains(TAB.NAV.ITEM.CLASS.ACTIVE) || isLocked) {
      return null;
    }
    ...
    _.each(TAB.NAV.ITEM.THIS, function (ELEMENT, KEY) {
...
function Form() {
  ...
  _.each(FORM.INPUT.ALL, function (ELEMENT) {
...
function Results() {
  ...
  function setHeight() {
    ...
    var resizeTime = setTimeout(function () {
      ...
      var resultsMaxHeight = getWindowSize().HEIGHT;
...
function Notifications() {
  ...
  var NOTIFICATION_DELAY = 300;

  //  Helpers
  var isFullNotification = false;
  var isNotificationReaded = false;
  ...
  function outsideClick(event) {
    if (!NOTIFICATION.CONT.ITEM.contains(event.target) && !NOTIFICATION.CONT.ITEM.parentNode.contains(event.target)) {
      hideNotification();
  ...
  function checkScrollPosition() {
    ...
    if (SCROLL.CURRENT > 0 && SCROLL.CURRENT + SCROLL.WINDOW_HEIGHT !== SCROLL.DOCUMENT_HEIGHT) {
    ...
    } else if (SCROLL.CURRENT > 0 && SCROLL.CURRENT + SCROLL.WINDOW_HEIGHT === SCROLL.DOCUMENT_HEIGHT) {
  ...
  function initNotification() {
    ...
    _.each(NOTIFICATION.BTN.READ, function (ELEMENT) {
...
function Table() {
  ...
  function checkAll() {
    if (!isCheckedAll) {
      _.each(TABLE.CHECKBOX, function (ELEMENT) {
